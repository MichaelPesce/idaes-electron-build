...
JSON_FRAMEWORK = {
    ...
    "build": {
        ...
        "deb": {
            "depends": ["libgfortran5", "libgomp1", "liblapack3", "libblas3"],
        },
        "flatpak": {},
        "files": ["dist/**/*", "build/**/*", "py_dist/**/*", "package.json", "main.js"],
...
def renameIcon(working_dir, project):
    try:
        icon_dir = os.path.join(
            working_dir,
            "../electron/idaes-flowsheet-processor-ui/frontend/public/icons",
        )
...
def generatePackageJson(
    project, ui_version, artifact_name, author, output_path="../electron/package.json"
):
    ...
    package_json["build"]["nsis"][
        "artifactName"
    ] = f"{artifact_name}_{ui_version}_win64.exe"
    package_json["build"]["win"][
        "artifactName"
    ] = f"{artifact_name}_{ui_version}_win64.exe"
    package_json["build"]["mac"][
        "artifactName"
    ] = f"{artifact_name}_{ui_version}_arm64.dmg"
    package_json["build"]["deb"][
        "artifactName"
    ] = f"{artifact_name}_{ui_version}_amd64.deb"
    package_json["build"]["linux"][
        "artifactName"
    ] = f"{artifact_name}_{ui_version}_amd64.deb"
...
def setEnvVariables(project, ui_version, project_version, additional_modules=""):
    working_dir = pathlib.Path(__file__).parent.resolve()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p",
        "--project",
        help="Project to create json file for. If not provided, default is WaterTAP.",
    )
    parser.add_argument(
        "-am",
        "--additional_modules",
        help="Comma-separated list of additional modules to include in build.",
        default="",
    )
    parser.add_argument(
        "-bv",
        "--build_version",
        help="Build version, typically in date format (yy.mm.dd). If not provided, will use current date.",
    )
    parser.add_argument(
        "-a",
        "--artifact_name",
        help="Artifact name. If not provided, will use provided project to create artifact name.",
    )
    parser.add_argument(
        "-pv",
        "--project_version",
        help="Project version, ie the version of WaterTAP, IDAES, or PROMMIS that is installed.",
    )
    ...
    if project not in valid_projects:
        print(
            f"project provided: {project} is not a valid project. Must be one of {valid_projects}. Defaulting to watertap"
        )
        project = "watertap"
    if project_version is None:
        print(
            f"project version is none. Attempting to get project version from python environment"
        )
        if project == "idaes":
